...
const { waitToAttack, getAttackInfo, assignUnit, assignCustomTool, getTotalAmountToolsFlank, getTotalAmountToolsFront, getAmountSoldiersFlank, getAmountSoldiersFront, getMaxUnitsInReinforcementWave, getCombinedTroops, assignTroops, getMaxAttackers, getShieldCount } = require("./attack")
...
xtHandler.on("pep", obj => {
    ...
    if (scoreShutoff > 0 && samsPoints >= scoreShutoff) {
        ...
        try {
            const attackInfo = await waitToAttack(async () => {
                ...
                if (pluginOptions.rejectOnShieldShortage) {
                    const totalShields = getShieldCount(unitInv, pluginOptions, [558], pluginOptions.customWaves)
                    if (totalShields < 5) {
                        throw "نقص دروع"
                    }
                }
                ...
                attackInfo.A.forEach((wave, index) => {
                    ...
                    }
                    {
                        ...
                        if (chestsMode !== 0) {
                            if (chestsMode === 1) {
                                attackerSamuraiTools.sort((a, b) =>
                                    Number(a.unitInfo.samuraiTokenBooster) - Number(b.unitInfo.samuraiTokenBooster))
                            } else {
                                attackerSamuraiTools.sort((a, b) =>
                                    Number(b.unitInfo.samuraiTokenBooster) - Number(a.unitInfo.samuraiTokenBooster))
                            }

                            const activeChests = attackerSamuraiTools.filter(t => Number(t.unitInfo?.samuraiTokenBooster || 0) > 0)
            ...
            switch (e) {
                ...
                case "NOT_ENOUGH_CURRENCY1":
                    console.debug("attackSamurai retryable:", e)
                    break
                default:
                    console.error(e)
                    quit = true
